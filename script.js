...
async function takePhoto(){
  ...
  if(!video || !classifier) return alert("Video atau AI belum siap!");
  if(video.videoWidth===0 || video.videoHeight===0) return alert("Tunggu sebentar, video belum siap");
  ...
  classifier.classify(img,(err,results)=>{
    if(err){ input.value="Gagal mengenali makanan"; console.error(err); return; }
...
window.addEventListener("DOMContentLoaded",async()=>{
  ...
  }catch(err){ alert("Gagal memuat model AI. Periksa koneksi internet."); console.error(err); }
});
